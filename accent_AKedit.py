...
words= sound.Sound('A', secs=-1, stereo=True, hamming=True,
    name='words')
words.setVolume(1.0)
...
for thisTrial in trials:
    ...
    words.setSound(thisTrial['file'], hamming=True)
    words.setVolume(1.0, log=False)
    
    # keep track of which components have finished
    wordsComponents = [word_resp, NZ]
    ...
    while continueRoutine:
        ...
        if words_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'words_resp.started')
        ...
        if words_resp.status == STARTED and not waitOnFlip:
            theseKeys = words_resp.getKeys(keyList=['a', 'l'], waitRelease=False)
            ...
            if len(_words_resp_allKeys):
                ...
                # was this correct?
                if (words_resp.keys == str(correct)) or (words_resp.keys == correct):
                    words_resp.corr = 1
                else:
                    words_resp.corr = 0
        ...
        if words.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.addData('words.started', tThisFlipGlobal)
            words.play(when=win)  # sync with win flip
    ...
    if words_resp.keys in ['', [], None]:  # No response was made
        ...
        if str(correct).lower() == 'none':
           words_resp.corr = 1;  # correct non-response
        else:
           words_resp.corr = 0;  # failed to respond (incorrectly)
    # store data for trials (TrialHandler)
    trials.addData('words_resp.keys',words_resp.keys)
    trials.addData('words_resp.corr',words_resp.corr)
    if words_resp.keys != None:  # we had a response
        trials.addData('words_resp.rt', words_resp.rt)
    words.stop()  # ensure sound has stopped at end of routine
